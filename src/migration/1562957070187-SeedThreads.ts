...
export class SeedThreads1562955773810 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        const usersQueryResult: ISelectIdQueryResult = await queryRunner.query('SELECT id FROM user');
        const sectionsQueryResult: ISelectIdQueryResult = await queryRunner.query('SELECT id FROM segment');

        const userIds: number[] = usersQueryResult.map(user => user.id);
        const sectionIds: number[] = sectionsQueryResult.map(section => section.id);
